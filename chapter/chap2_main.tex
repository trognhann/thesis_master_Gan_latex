...
\begin{figure}[H]
    ...
    \caption{Tổng quan kiến trúc DTGAN (AnimeGANv3): Ảnh đầu vào qua Shared Base Encoder để tạo các bản đồ đặc trưng $x_0, x_1, x_2, x_3$, sau đó được xử lý song song bởi Support Tail và Main Tail với skip connections kiểu U-Net. Output của Support Tail qua Guided Filter trước khi đưa vào Discriminator grayscale \cite{Liu2024}.}
    \label{fig:dtgan_architecture}
\end{figure}

Cụ thể, kiến trúc Generator của DTGAN bao gồm ba thành phần chính:
\begin{itemize}
    \item \textbf{Base Encoder (chia sẻ):} Trích xuất đặc trưng từ ảnh đầu vào, downsampling 3 lần để tạo feature map $32 \times 32$.
    \item \textbf{Support Tail:} Decoder sinh ảnh anime thô $\widehat{y}_s$ (\texttt{fake\_s}), được lọc qua Guided Filter tạo $\widehat{y}$ (\texttt{generated}), dùng để huấn luyện với Discriminator grayscale và tập dữ liệu ảnh anime.
    \item \textbf{Main Tail:} Decoder tinh chỉnh ảnh $\widehat{y}_m$ (\texttt{fake\_m}), dùng pseudo ground-truth từ NL-Means + L0 Smoothing để học chi tiết mịn.
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Cơ chế hoạt động của LADE: feature map $x$ được chiếu qua Conv $1\times 1$ để tạo thống kê thích ứng $(\mu_t, \sigma_t)$, sau đó bình thường hóa Instance rồi tái tỷ lệ hóa thích ứng. Bảng so sánh cho thấy LADE tự học tham số từ chính dữ liệu, không cần ảnh phong cách riêng như AdaIN \cite{Liu2024}.}
    \label{fig:lade}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Kiến trúc External Attention v3 sử dụng trong DTGAN tại vị trí bottleneck $32 \times 32$. Hai memory bank học được $M_k \in \mathbb{R}^{C \times k}$ và $M_v \in \mathbb{R}^{k \times C}$ cho phép mô hình học phụ thuộc toàn cục với độ phức tạp $O(N \cdot k)$ thay vì $O(N^2)$ của Self-Attention \cite{Guo2021, Liu2024}.}
...
\begin{itemize}
    \item \textbf{Discriminator (Support D):} Nhận ảnh grayscale làm đầu vào. Phân biệt giữa ảnh anime grayscale thật và $\widehat{y}$ (\texttt{generated}) grayscale từ Support Tail.
    \item \textbf{Discriminator\_main (Main D):} Nhận ảnh màu đầy đủ. Phân biệt giữa ảnh đã qua NL-Means + L0 Smoothing thật và $\widehat{y}_m$ từ Main Tail.
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Pipeline huấn luyện hai giai đoạn và hệ thống hàm mất mát của DTGAN. Giai đoạn 1 (pre-training) chỉ dùng Content Loss. Giai đoạn 2 kết hợp nhiều thành phần loss cho Support Tail và Main Tail, với pseudo ground-truth được tạo bởi CPU preprocessing \cite{Liu2024}.}
...
Chương này đã trình bày chi tiết kiến trúc kỹ thuật của AnimeGANv3 (DTGAN), bao gồm:

\begin{itemize}
    \item Kiến trúc Generator hai nhánh (Double-Tail): Base Encoder dùng chung, Support Tail học đường nét anime thô, Main Tail tinh chỉnh chi tiết mịn.
    \item Kỹ thuật chuẩn hóa LADE: tự học tham số thích ứng từ feature map, vượt qua hạn chế của Instance Normalization và AdaIN.
    \item External Attention v3: giảm độ phức tạp từ $O(N^2)$ xuống $O(Nk)$ nhờ memory bank học được.
    \item Hệ thống hàm mất mát đa thành phần: Adversarial (LSGAN), Content, Style (Decentralized Gram), Region Smoothing, Lab Color, TV Loss cho Support Tail; Fine-grained Revision Loss cho Main Tail.
    \item Quy trình huấn luyện hai giai đoạn với CPU preprocessing song song.
\end{itemize}

Chương 3 sẽ trình bày kết quả thực nghiệm và đánh giá định lượng, định tính của mô hình DTGAN trên các bộ dữ liệu phong cách Hayao và Shinkai.

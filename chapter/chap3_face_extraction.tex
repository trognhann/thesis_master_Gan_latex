...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{Face Mode:} Chỉ vùng khuôn mặt sau mở rộng được resize về $256 \times 256$ và đưa qua Generator.
...
\end{figure}
    \noindent\textit{Chú thích:} Face Mode: ảnh qua RetinaFace $\to$ Margin Expansion $\to$ Crop $256\times256$ $\to$ ONNX Inference. Landscape Mode: ảnh resize giữ tỉ lệ (chia hết 8) rồi trực tiếp qua ONNX Inference. Hybrid Mode: nền qua Landscape, mỗi khuôn mặt qua Face Mode rồi ghép feathered blend \cite{Liu2024}.
...
\begin{enumerate}
    ...
    \item Duy trì tỉ lệ khung hình 1:1 (vuông), phù hợp với yêu cầu đầu vào $256 \times 256$ của mô hình --- tránh biến dạng hình học do resize không đồng đều hai chiều.
\end{enumerate}
...
\textbf{Tính chất 2 --- Vùng cắt vuông không được bảo đảm tuyệt đối.} Bước 4 đặt $h' = w'$, nhưng Bước 5 lại áp thêm các toán tử $\max(0,\cdot)$ và $\min(H,\cdot)$. Nếu khuôn mặt nằm gần biên trên hoặc biên dưới, chiều cao thực tế sẽ nhỏ hơn $h'$ và vùng cắt trở thành hình chữ nhật. Khi đó, phép \texttt{resize} về $256\times256$ sẽ gây biến dạng tỉ lệ khung hình. Đây là một hạn chế đã biết của cài đặt hiện tại; hướng khắc phục là đệm thêm viền (padding) thay vì cắt cụt, sẽ được bàn ở Mục~\ref{subsec:pipeline_limitations}.

\textbf{Tính chất 3 --- Bề rộng khuôn mặt sau chuẩn hóa là hằng số.} Kết hợp Tính chất 1 với bước resize về $256\times256$, khuôn mặt trong ảnh đưa vào Generator luôn có bề rộng:
\begin{equation}
\label{eq:face_width_after}
w_{\text{model}} = 256 \cdot \frac{w}{w'} = \frac{256}{1 + 2m} = 128 \text{ pixel} \quad \text{khi } m = 0{,}5
\end{equation}
...
\begin{itemize}
    ...
    \item Khi $m$ \textbf{quá lớn}, lỗi mang tính \textit{pha loãng}: khuôn mặt chỉ còn chiếm $256/(1+2m) \approx 107$ pixel bề rộng khi $m = 0{,}7$, đồng thời nền phức tạp chiếm phần lớn diện tích và chi phối các thống kê phong cách mà LADE cùng External Attention học được (xem Chương~2, Mục~\ref{subsec:ext_attention_arch}).
\end{itemize}
...
\label{eq:res_face}
w_{\text{eff}}^{\text{Face}} &= \frac{256}{1 + 2m} = 128 \ \text{px} \qquad \text{(theo Tính chất 3, Mục~\ref{subsec:margin_properties})}
\end{align}
...
\begin{equation}
\label{eq:kappa}
\boxed{\ \kappa = \frac{w_{\text{eff}}^{\text{Face}}}{w_{\text{eff}}^{\text{Landscape}}} = \frac{128}{s \cdot w}\ }
\end{equation}

\subsection{Diễn giải và điểm hòa vốn}
\label{subsec:eff_res_discussion}

Công thức~(\ref{eq:kappa}) cho một kết luận rõ ràng: \textbf{Face Mode chỉ mang lại lợi ích về độ phân giải khi khuôn mặt hẹp hơn 128~pixel trong ảnh đã giới hạn kích thước}. Bảng~\ref{tab:effective_resolution} minh họa bằng các tình huống điển hình.
...
\hline
Ảnh nhóm, chụp toàn thân & 40 & 40 px & 128 px & \textbf{3,2$\times$} \\
\hline
Chân dung bán thân & 80 & 80 px & 128 px & \textbf{1,6$\times$} \\
\hline
\textit{Điểm hòa vốn} & \textit{128} & \textit{128 px} & \textit{128 px} & \textit{1,0$\times$} \\
\hline
Chân dung cận cảnh & 300 & 300 px & 128 px & 0,43$\times$ \\
\hline
\end{tabular}
\end{table}
...
\hline
Kích thước input & $256 \times 256$ & Giữ tỉ lệ, chia hết 8 & Nền: chia hết 8; Mặt: $256^2$ \\
\hline
...
\begin{enumerate}
    ...
    \item \textbf{[Face Mode] Cắt ảnh:} \texttt{img = img[y1:y2, x1:x2]}
    \item \textbf{Resize cho model:}
        \begin{itemize}
            \item Face mode: \texttt{img.resize((256, 256))}
...
Ngoài hai chế độ Face Mode và Landscape Mode, hệ thống còn hỗ trợ chế độ \textbf{Hybrid Mode} --- kết hợp ưu điểm của cả hai để xử lý ảnh nhóm người, trong đó cần giữ cả chất lượng nền lẫn chi tiết khuôn mặt cao. Đây là chế độ phức tạp nhất về mặt tính toán nhưng cho kết quả toàn diện nhất.
...
\begin{enumerate}
    ...
    \item \textbf{Suy luận từng khuôn mặt (Face Pass):} Với \textbf{mỗi} khuôn mặt phát hiện được (không chỉ khuôn mặt lớn nhất như Face Mode), áp dụng Margin Expansion, cắt vùng mặt từ ảnh giới hạn kích thước, resize về $256 \times 256$ và suy luận riêng qua cùng mô hình ONNX.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figChap3/hybrid_mode_pipeline.png}
...
\For{$b \in \mathcal{B}$}
    ...
    \State $C \gets \textsc{Cắt}(I_{\text{lim}},\ b')$; \quad $C \gets \textsc{Resize}(C,\ 256 \times 256)$
...
Ảnh khuôn mặt sau suy luận (kích thước $256 \times 256$) được resize về $(w_f, h_f)$ bằng thuật toán nội suy Lanczos trước khi ghép. Lanczos được chọn thay vì bilinear vì nó dùng nhân lọc dựa trên hàm sinc, bảo toàn tốt hơn các thành phần tần số cao --- chính là những đường nét sắc mà toàn bộ pipeline đã nỗ lực tạo ra. Việc dùng một bộ nội suy làm mờ ở bước cuối sẽ triệt tiêu phần lớn lợi ích của các bước trước đó.
...
Khi ghép trực tiếp vùng mặt (xử lý Face Mode ở $256 \times 256$) vào nền (xử lý Landscape ở độ phân giải khác), sẽ xuất hiện đường biên rõ ràng do khác biệt về phong cách và độ phân giải giữa hai vùng. Kỹ thuật \textbf{feathered blending} sử dụng mặt nạ alpha với viền mờ Gaussian để tạo vùng chuyển tiếp mượt mà.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{figChap3/feathered_blending.png}
...
Gọi $T_L$ là thời gian suy luận Landscape, $T_F$ là thời gian suy luận Face ($256 \times 256$), $T_D$ là thời gian phát hiện mặt, $T_{\text{blend}}$ là thời gian tạo mặt nạ và ghép, $N$ là số khuôn mặt phát hiện được. Tổng thời gian xử lý Hybrid Mode:
...
Chi phí tính toán của Hybrid Mode tăng \textbf{tuyến tính} theo số khuôn mặt. Tuy nhiên, do $T_F$ sử dụng kích thước đầu vào cố định $256 \times 256$ --- thường nhỏ hơn ảnh Landscape --- thời gian mỗi lần suy luận Face thường nhanh hơn suy luận Landscape. Trong thực nghiệm (Chương~4), ảnh có 2--3 khuôn mặt thường hoàn thành Hybrid Mode trong thời gian chấp nhận được cho ứng dụng tương tác.
...
\begin{itemize}
    ...
    \item \textbf{5 facial landmarks:} Thông tin landmarks (dù chưa được khai thác trong phiên bản hiện tại) mở ra khả năng face alignment trong các phiên bản tương lai, không tốn thêm chi phí tính toán.
...
Bảng~\ref{tab:face_vs_landscape} tóm tắt sự đánh đổi giữa ba chế độ. Face Mode cho chất lượng anime cao nhất đối với ảnh chân dung do ba cơ chế đã phân tích ở Mục~\ref{subsec:eff_res_discussion}: chuẩn hóa độ phân giải khuôn mặt về 128~pixel, khớp phân phối huấn luyện $256\times256$, và loại bỏ ảnh hưởng của nền lên các cơ chế thống kê toàn cục.
...
\begin{enumerate}
    ...
    \item \textbf{Vùng cắt không vuông gây biến dạng.} Như đã nêu ở Tính chất 2 (Mục~\ref{subsec:margin_properties}), khuôn mặt gần biên trên/dưới cho vùng cắt hình chữ nhật, và phép resize về $256\times256$ làm biến dạng tỉ lệ. Giải pháp là đệm viền bằng màu trung bình hoặc phản chiếu gương thay vì cắt cụt.
    \item \textbf{Không khai thác landmark.} Năm điểm đặc trưng sẵn có không được dùng cho căn chỉnh khuôn mặt --- bỏ lỡ cơ hội cải thiện đúng nhóm ảnh khó nhất.
...
\begin{itemize}
    ...
    \item \textbf{Thuật toán Margin Expansion} 6 bước (Thuật toán~\ref{alg:margin_expansion}): mở rộng bounding box đối xứng, duy trì tỉ lệ 1:1, xử lý phân nhánh khi $h' < h$ và các trường hợp khuôn mặt gần biên ảnh. Tính chất quan trọng nhất được rút ra: với $m = 0{,}5$, khuôn mặt luôn được đưa vào Generator ở bề rộng cố định \textbf{128 pixel}, không phụ thuộc kích thước ảnh gốc.
    \item \textbf{Phân tích độ phân giải hiệu dụng:} Hệ số lợi ích $\kappa = 128/(s \cdot w)$ cho thấy Face Mode mang lại lợi ích về độ phân giải khi khuôn mặt hẹp hơn 128~pixel trong ảnh đã giới hạn kích thước, và lợi ích lớn nhất ở ảnh nhóm hoặc ảnh chụp xa. Hai cơ chế bổ sung --- khớp phân phối huấn luyện và loại bỏ nhiễu nền --- đóng góp độc lập với kích thước khuôn mặt.

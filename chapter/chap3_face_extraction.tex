...
Để hệ thống trích xuất và chuyển đổi hoạt động liền mạch với hiệu năng thời gian thực, luận văn đã thiết kế một luồng xử lý dữ liệu (pipeline) nội bộ hỗ trợ \textbf{ba chế độ vận hành}:
...
\end{figure}
    \noindent\textit{Chú thích:} Face Mode: ảnh qua RetinaFace $\to$ Margin Expansion $\to$ Crop $256\times256$ $\to$ ONNX Inference. Landscape Mode: ảnh resize giữ tỉ lệ (chia hết 8) rồi trực tiếp qua ONNX Inference. Hybrid Mode: nền qua Landscape, mỗi khuôn mặt qua Face Mode rồi ghép feathered blend \cite{AnimeGANv3}.
...
\begin{enumerate}
    ...
    \item \textbf{Non-Maximum Suppression (NMS):} Loại bỏ các bounding box trùng lặp với $\text{IoU} > 0.3$ (tham số \texttt{nms\_threshold}): giữ lại box có confidence cao nhất trong mỗi nhóm box chồng lấp.
...
Trong chế độ Face Mode, phần tử đầu tiên (\texttt{bboxes[0]}) -- khuôn mặt lớn nhất -- được chọn làm đối tượng chuyển đổi. Trong chế độ Hybrid Mode, toàn bộ danh sách khuôn mặt sau lọc đều được xử lý. Nếu kết quả rỗng (\texttt{dets.shape} chứa 0), hàm trả về \texttt{None} và pipeline chuyển sang Landscape Mode.
...
Kỹ thuật \textbf{Margin Expansion} (mở rộng lề) được phát triển trong luận văn để giải quyết vấn đề này: tự động mở rộng bounding box theo tỉ lệ phần trăm kích thước mặt, đồng thời duy trì tỉ lệ khung hình 1:1 (vuông) phù hợp với yêu cầu đầu vào $256 \times 256$ của mô hình.
...
\textbf{Quy tắc resize Landscape mode:} Kích thước ảnh phải chia hết cho 8 (do kiến trúc encoder 3 lần downsampling stride-2 của Generator), đồng thời có ngưỡng tối thiểu 256 pixel để đảm bảo chất lượng đầu ra. Hàm \texttt{to\_8s(x)}:
\begin{equation}
\label{eq:to8s}
...
Ngoài hai chế độ Face Mode và Landscape Mode, hệ thống còn hỗ trợ chế độ \textbf{Hybrid Mode} -- kết hợp ưu điểm của cả hai chế độ để xử lý ảnh nhóm người trong đó cần giữ cả chất lượng nền lẫn chi tiết khuôn mặt cao. Đây là chế độ phức tạp nhất về mặt tính toán nhưng cho kết quả toàn diện nhất.
...
\begin{enumerate}
    \item \textbf{Phát hiện khuôn mặt:} Gọi \texttt{detect\_face()} trên ảnh đã giới hạn kích thước ($\leq 1280$ pixel cạnh dài). Nếu không phát hiện khuôn mặt nào (\texttt{bboxes = None}), tự động chuyển sang Landscape Mode (graceful degradation) và chỉ thực hiện một lần suy luận duy nhất.
...
s = \begin{cases} \frac{1280}{\max(W, H)} & \text{nếu } \max(W, H) > 1280 \\ 1.0 & \text{ngược lại} \end{cases}
\end{equation}
...
Ảnh khuôn mặt sau suy luận (kích thước $256 \times 256$) được resize về $(w_f, h_f)$ bằng thuật toán nội suy Lanczos trước khi ghép.

\subsection{Kỹ thuật Feathered Blending}
\label{subsec:feathered_blending}

Khi ghép trực tiếp vùng mặt (xử lý Face Mode ở $256 \times 256$) vào nền (xử lý Landscape ở độ phân giải thấp hơn), sẽ xuất hiện đường biên rõ ràng do khác biệt phong cách và độ phân giải giữa hai vùng. Kỹ thuật \textbf{feathered blending} sử dụng mặt nạ alpha với viền mờ Gaussian để tạo vùng chuyển tiếp mượt mà.
...
\textbf{Bước 1 -- Tạo mặt nạ nền đen:} Khởi tạo ảnh grayscale kích thước $(w_f, h_f)$ với tất cả pixel có giá trị 0 (đen hoàn toàn):
\begin{equation}
\label{eq:mask_init}
...
\hat{M} = M * G_\delta, \quad G_\delta(x,y) = \frac{1}{2\pi\delta^2} e^{-\frac{x^2+y^2}{2\delta^2}}
\end{equation}
...
trong đó $I_{\text{face}}$ là ảnh khuôn mặt đã suy luận (resize về $w_f \times h_f$), $I_{\text{bg}}$ là ảnh nền anime tại vùng $(x_1^{\text{orig}}, y_1^{\text{orig}})$, và $\hat{M}$ là mặt nạ alpha đã làm mờ. Hệ số $\delta = 8\%$ kích thước cạnh nhỏ hơn của vùng mặt được chọn qua thực nghiệm: nhỏ hơn sẽ tạo đường viền rõ, lớn hơn sẽ làm mờ quá nhiều chi tiết khuôn mặt.

\subsection{Phân tích Độ phức tạp}
\label{subsec:hybrid_complexity}

Gọi $T_L$ là thời gian suy luận Landscape và $T_F$ là thời gian suy luận Face ($256 \times 256$), $T_D$ là thời gian phát hiện mặt, $N$ là số khuôn mặt phát hiện được. Tổng thời gian xử lý Hybrid Mode:

\begin{equation}
\label{eq:hybrid_time}
T_{\text{hybrid}} = T_D + T_L + N \cdot T_F + N \cdot T_{\text{blend}}
\end{equation}
...
Chi phí tính toán của Hybrid Mode tăng tuyến tính theo số khuôn mặt. Tuy nhiên, do $T_F$ sử dụng kích thước đầu vào cố định $256 \times 256$ (thường nhỏ hơn ảnh Landscape), thời gian mỗi lần suy luận Face thường nhanh hơn suy luận Landscape. Trong thực nghiệm (Chương~4), ảnh có 2--3 khuôn mặt thường hoàn thành Hybrid Mode trong thời gian chấp nhận được cho ứng dụng tương tác.
...
Pipeline được thiết kế theo nguyên tắc \textbf{graceful degradation}: nếu không phát hiện được khuôn mặt (confidence dưới ngưỡng 0.8, hoặc khuôn mặt bị che khuất), hệ thống tự động chuyển sang chế độ Landscape để xử lý toàn bộ ảnh, tránh trả về lỗi cho người dùng. Cơ chế này áp dụng cho cả Face Mode lẫn Hybrid Mode.

Bảng~\ref{tab:face_vs_landscape} tóm tắt sự đánh đổi giữa ba chế độ. Face Mode cho chất lượng anime cao nhất đối với ảnh chân dung do:
\begin{enumerate}
    \item Khuôn mặt chiếm 100\% diện tích ảnh đầu vào $\Rightarrow$ model tập trung toàn bộ năng lực vào vùng mặt.
    \item Loại bỏ background phức tạp giúp tránh style transfer không nhất quán.
    \item Cố định kích thước $256 \times 256$ cho phép model hoạt động ở chế độ tối ưu.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Pipeline end-to-end} tích hợp ba chế độ Face Mode, Landscape Mode và Hybrid Mode, với cơ chế graceful degradation tự động chuyển chế độ khi không phát hiện khuôn mặt.
    \item \textbf{Luồng dữ liệu qua RetinaFace:} tiền xử lý ImageNet $\to$ FPN đa tỉ lệ $\to$ giải mã Prior Box $\to$ Top-K $\to$ NMS $\to$ sắp xếp diện tích.
    \item \textbf{Thuật toán Margin Expansion} 6 bước: mở rộng bounding box 50\% mỗi chiều, duy trì tỉ lệ 1:1, xử lý phân nhánh khi $h' < h$ và edge cases khuôn mặt gần biên ảnh.
    \item \textbf{Hybrid Mode với Feathered Blending:} xử lý nền Landscape và từng khuôn mặt Face Mode riêng biệt, ghép bằng alpha mask Gaussian.
    \item \textbf{Luồng dữ liệu qua Generator ONNX:} chuẩn hóa $[-1, 1]$, suy luận, hậu chuẩn hóa $[0, 255]$; hỗ trợ cả \texttt{to\_8s} và \texttt{to\_16s} cho các biến thể model.
\end{itemize}
